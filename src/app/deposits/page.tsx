...
export default function MasterTransactionPage() {
    ...
    const [type, setType] = useState<'deposit' | 'withdraw'>('deposit'); // Toggle state
    ...
    const saveEdit = (id: string) => {
        updateTransaction(id, { title: editTitle, amount: Number(editAmount) });
        setEditingId(null);
    };
        ...
        <div className="max-w-5xl mx-auto space-y-10 p-4">
            ...
            <div className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-xl">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
                    ...
                    {/* Toggle Switch to help user understand */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                    <div className="md:col-span-6 space-y-2">
                        ...
                        <input
                            ...
                            className="w-full p-4 rounded-2xl bg-gray-50 border-none ring-1 ring-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none text-lg"
                                ...
                                <tr key={t.id} className="hover:bg-gray-50/30 transition-colors">
                                    <td className="p-6 font-bold text-gray-800">
                                        {editingId === t.id ? (
                                            <input
                                                ...
                                                className="p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none w-full"
                                            />
                                        ) : t.title}
                                    </td>
                                    <td className="p-6">
                                    ...
                                    <td className={`p-6 font-extrabold text-lg ${t.type === 'deposit' ? 'text-green-600' : 'text-red-600'}`}>
                                        {editingId === t.id ? (
                                            <input
                                                ...
                                                className="p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none w-24"
                                            />
                                        ) : (
                                    ...
                                    <td className="p-6 text-right">
                                        <div className="flex justify-end gap-2">
                                            {editingId === t.id ? (
                                                <>
                                                    <button onClick={() => saveEdit(t.id)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg"><Check size={20} /></button>
                                                    <button onClick={() => setEditingId(null)} className="p-2 text-gray-400 hover:bg-gray-100 rounded-lg"><X size={20} /></button>
                                                </>
                                            ) : (
                                                <>
                                                    <button onClick={() => startEdit(t)} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg"><Edit2 size={18} /></button>
                                                    <button onClick={() => deleteTransaction(t.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={18} /></button>
                                                </>
                                            )}
                                        </div>
                                    </td>
